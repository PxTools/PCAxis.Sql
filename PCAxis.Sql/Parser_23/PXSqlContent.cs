...
namespace PCAxis.Sql.Parser_23
{
    public class PXSqlContent : IComparable<PXSqlContent>
    {
        ...
        private PXSqlNpm.NPMCharacter mNPMcharacterinMissingRows = null;

        internal PXSqlNpm.NPMCharacter NPMcharacterinMissingRows
        {
            get { return mNPMcharacterinMissingRows; }
        }
        ...
        public void ParseMeta(PCAxis.Paxiom.IPXModelParser.MetaHandler handler, StringCollection LanguageCodes)
        {
            ...
            // META_ID
            if (!String.IsNullOrEmpty(this.mRow.MetaId))
            {
                values.Clear();
                values.Add(this.mRow.MetaId);
                handler(PXKeywords.META_ID, noLanguage, PXSqlMeta_23.mContVariableCode + "\",\"" + subkey, values);
            }
            ...
            if (!String.IsNullOrEmpty(this.mPresDecimals))
            {
                ...
                String myKey = PXSqlMeta_23.mContVariableCode + "\",\"" + subkey;//for PXModelBuilder.SplittString   A","B","C 
                                                                                 // not "A","B","C" 
         ...
         */
        private string PaxiomifyStockFA(string StockFA, SqlDbConfig_23 config)
        {
            string myOut = "";
        ...
        private string PaxiomifyCFPrices(string CFPrices, SqlDbConfig_23 config)
        {
            string myOut = "";
