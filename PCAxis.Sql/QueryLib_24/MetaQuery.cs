...
namespace PCAxis.Sql.QueryLib_24
{
    ...
    public partial class MetaQuery
    {
        ...
        private readonly SqlDbConfig_24 mDbConfig;
        public SqlDbConfig_24 DB
        {
        ...
        public PxSqlCommand GetPxSqlCommand()
        {
            ...
            {
                return new PCAxis.Sql.SqlClientCleanup.PxSqlCommandNoTempTables(mSelectedDbInfo.DataBaseType, mSelectedDbInfo.DataProvider, mSelectedDbInfo.ConnectionString);
            }
        }
        ...
        public ValueRow2HMDictionary GetValueRowDictionary(string mainTable, StringCollection subTables, string variable, StringCollection someValueCodes, string valueExtraExists)
        {
            ...
            stopWatch.Start();
            logTime.Debug("Start " + System.Reflection.MethodBase.GetCurrentMethod().Name + " bad");
#endif
        ...
        public List<RelevantFootNotesRow> GetRelevantFoonotes(string mainTable)
        {
            ...
            string oraninja1 = this.mSqlCommand.getOracleNinja1();
            string oraninja2 = this.mSqlCommand.getOracleNinja2();
           ...
           DB.Footnote.FootnoteTextCol.ForSelect();
            ...
            "union " +
              "(select distinct " + oraninja1 + " dvt." + DB.SubTableVariable.MainTableCol.PureColumnName() + " as MainTable, fi." + DB.FootnoteValue.FootnoteNoCol.PureColumnName() + " as  FootNoteNo, '6' as FootNoteType, '*' as Contents, dvt." + DB.SubTableVariable.VariableCol.PureColumnName() + " as Variable, fi." + DB.FootnoteValue.ValuePoolCol.PureColumnName() + " as ValuePool, fi." + DB.FootnoteValue.ValueCodeCol.PureColumnName() + " as ValueCode, " +
                      "'*' as TimePeriod, '*' as SubTable " +
               "from " + Md + DB.FootnoteValue.TableName + " fi, " + Md + DB.SubTableVariable.TableName + " dvt, " + Md + DB.ValueSetGrouping.TableName + " vmg, " + Md + DB.Grouping.TableName + " g, " + Md + DB.ValueGroup.TableName + " vg " +
               "where  dvt. " + DB.ValueSet.ValueSetCol.PureColumnName() + " = vmg." + DB.ValueSetGrouping.ValueSetCol.PureColumnName() +
                 " and  vmg." + DB.ValueSetGrouping.GroupingCol.PureColumnName() + " = g." + DB.Grouping.GroupingCol.PureColumnName() +
                 " and  g." + DB.Grouping.GroupingCol.PureColumnName() + " = vg." + DB.ValueSetGrouping.GroupingCol.PureColumnName() +
                 " and  g." + DB.Grouping.ValuePoolCol.PureColumnName() + " = vg." + DB.ValueGroup.ValuePoolCol.PureColumnName() +
                " and  vg." + DB.ValueGroup.ValuePoolCol.PureColumnName() + " = fi." + DB.FootnoteValue.ValuePoolCol.PureColumnName() +
                " and  vg." + DB.ValueGroup.GroupCodeCol.PureColumnName() + " = fi." + DB.FootnoteValue.ValueCodeCol.PureColumnName() + oraninja2 + ") " +
            "union " +
        ...
        const int bucketSize = 500; //bucket size can possibly be tuned but 500 is probably good for performance

        private IEnumerable<IEnumerable<string>> CreateGroupCodeBuckets(IEnumerable<string> groupCodes)
        {
            if (groupCodes == null) return null;

            var result = new List<List<string>>();
            var groupCodeArray = groupCodes.ToArray();
            List<string> bucket = new List<string>();

            if (groupCodeArray.Length <= bucketSize)
            {
                bucket.AddRange(groupCodeArray);
                result.Add(bucket);
                return result;
            }
        ...
        public Dictionary<string, ValueGroupRow> GetValueGroupRowskeyValueCode(string aGrouping, IEnumerable<string> groupCodes, bool emptyRowSetIsOK)
        {
            ...
            foreach (var bucket in buckets)
            {
                ...
                {
                    myOut[keyValueItem.Key] = keyValueItem.Value;
                }
            }
        ...
        private Dictionary<string, ValueGroupRow> GetValueGroupRowskeyValueCodeFromBucket(string aGrouping, IEnumerable<string> groupCodesBucket, bool emptyRowSetIsOK)
        {
            ...
            //
            var bucketArray = groupCodesBucket.ToArray();

            sqlString += " WHERE " + DB.ValueGroup.GroupingCol.Is(mSqlCommand.GetParameterRef("aGrouping")) +
                         " AND " + DB.ValueGroup.GroupCodeCol.In(mSqlCommand.GetParameterRef("aGroupCode"), bucketArray.Length);

            // creating the parameters
            System.Data.Common.DbParameter[] parameters = new System.Data.Common.DbParameter[bucketArray.Length + 1];
            parameters[0] = mSqlCommand.GetStringParameter("aGrouping", aGrouping);

            for (int i = 1; i < bucketArray.Length + 1; i++)
            {
                parameters[i] = mSqlCommand.GetStringParameter(string.Format("aGroupCode{0}", i), bucketArray[i - 1]);
            }

            List<string> foundGroupCodes = new List<string>();

            using (DataSet ds = mSqlCommand.ExecuteSelect(sqlString, parameters))
            {
                DataRowCollection myRows = ds.Tables[0].Rows;
            ...
            if (!emptyRowSetIsOK && foundGroupCodes.Count() != groupCodesBucket.Count())
            {
                var missingGrouCodes = groupCodesBucket.Except(foundGroupCodes);
                throw new PCAxis.Sql.Exceptions.DbException(35, " Grouping = " + aGrouping + " missingGrouCodes = " + string.Join(",", missingGrouCodes));
            }
